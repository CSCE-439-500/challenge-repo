...
def save_checkpoint(
    filepath: str, output_dir: str = None, base_name: str = None
) -> str:
    ...
    Args:
        ...
        base_name: Optional base name for checkpoint (defaults to file basename)
    ...
    try:
        ...
        if output_dir:
            ...
            # Use provided base_name or fall back to file basename
            if base_name:
                filename = base_name
            else:
            ...
            checkpoint_path = os.path.join(
                checkpoints_dir, f"{base_name_only}_checkpoint_{timestamp}{extension}"
            )
        else:
            # Fallback to original behavior
            if base_name:
                base_name_only = os.path.splitext(base_name)[0]
                extension = os.path.splitext(base_name)[1]
            else:
                base_name_only = os.path.splitext(filepath)[0]
                extension = os.path.splitext(filepath)[1]
            checkpoint_path = f"{base_name_only}_checkpoint_{timestamp}{extension}"
...
def list_checkpoints(base_path: str, checkpoints_dir: str = None) -> list:
    """List all checkpoint files for a given base file.

    Args:
        base_path: Base path of the file (without extension) or just filename
        checkpoints_dir: Optional directory to search for checkpoints
    ...
    try:
        ...
        if checkpoints_dir and os.path.exists(checkpoints_dir):
            # Search in specific checkpoints directory
            for filename in os.listdir(checkpoints_dir):
                if filename.startswith(base_file) and "_checkpoint_" in filename:
                    checkpoints.append(os.path.join(checkpoints_dir, filename))
        else:
            # Original behavior - search in same directory as base file
