...
class PEMimicryEngine:
    ...
    def __init__(self, template_db_path: Optional[str] = None):
        ...
        logger.info("action=mimicry_engine_initialized templates=%d", len(self.templates))
    
    def _load_templates(self) -> None:
        """Load benign PE templates from database."""
        if not os.path.exists(self.template_db_path):
            logger.warning("action=template_db_not_found path=%s", self.template_db_path)
            ...
            BenignTemplate(
                ...
                characteristics={
                    ...
                    "sections": {
                        ...
                        ".rdata": {"characteristics": 0x40000040, "is_executable": False},
                        ".data": {"characteristics": 0xc0000040, "is_executable": False},
                        ".rsrc": {"characteristics": 0x40000040, "is_executable": False}
                    },
                    "imports": {
                        "kernel32.dll": ["GetModuleHandleW", "GetProcAddress", "LoadLibraryW"],
                        "user32.dll": ["CreateWindowExW", "DefWindowProcW", "DispatchMessageW"],
                ...
                common_functions=["GetModuleHandleW", "GetProcAddress", "CreateWindowExW"]
            ),
            ...
            BenignTemplate(
                ...
                characteristics={
                    ...
                    "sections": {
                        ...
                        ".rdata": {"characteristics": 0x40000040, "is_executable": False},
                        ".data": {"characteristics": 0xc0000040, "is_executable": False},
                        ".pdata": {"characteristics": 0x40000040, "is_executable": False},
                        ".rsrc": {"characteristics": 0x40000040, "is_executable": False}
                    },
                    "imports": {
                        "kernel32.dll": ["CreateProcessW", "GetCurrentProcess", "ExitProcess"],
                ...
                common_functions=["CreateProcessW", "GetCurrentProcess", "RegOpenKeyExW"]
            ),
    ...
    def find_similar_template(self, pe_characteristics: Dict[str, Any], 
                            category: Optional[str] = None) -> Optional[BenignTemplate]:
        ...
        logger.info("action=template_matched template=%s score=%.2f", 
                   best_template.name if best_template else "none", best_score)
        
        return best_template
    
    def _calculate_similarity_score(self, pe_chars: Dict[str, Any], 
                                  template: BenignTemplate) -> float:
        ...
        if "header" in pe_chars and "header" in template.characteristics:
            header_score = self._compare_headers(
                pe_chars["header"], 
                template.characteristics["header"]
            )
        ...
        if "sections" in pe_chars and "sections" in template.characteristics:
            section_score = self._compare_sections(
                pe_chars["sections"], 
                template.characteristics["sections"]
            )
        ...
        if "imports" in pe_chars and "imports" in template.characteristics:
            import_score = self._compare_imports(
                pe_chars["imports"], 
                template.characteristics["imports"]
            )
    ...
    def _compare_headers(self, pe_header: Dict[str, Any], 
                        template_header: Dict[str, Any]) -> float:
    ...
    def _compare_sections(self, pe_sections: Dict[str, Any], 
                         template_sections: Dict[str, Any]) -> float:
    ...
    def _compare_imports(self, pe_imports: Dict[str, List[str]], 
                        template_imports: Dict[str, List[str]]) -> float:
    ...
    def generate_mimicry_plan(self, pe_characteristics: Dict[str, Any], 
        ...
        for section_name in template_sections:
            if section_name not in current_sections:
                mimicry_plan["modifications"]["section_additions"].append({
                    "name": section_name,
                    "characteristics": template.characteristics["sections"].get(section_name, {})
                })
        ...
        mimicry_plan["modifications"]["string_additions"] = self._generate_benign_strings(template)
        
        logger.info("action=mimicry_plan_generated template=%s modifications=%d", 
                   template.name, len(mimicry_plan["modifications"]))
    ...
    def _generate_benign_strings(self, template: BenignTemplate) -> List[str]:
        ...
        category_strings = {
            "system_utility": [
                "Windows", "System", "Utility", "Configuration", "Settings",
                "Error", "Warning", "Information", "Success", "Failed"
            ],
            "web_browser": [
                "Chrome", "Browser", "Internet", "Web", "URL", "HTTP", "HTTPS",
                "Security", "Privacy", "Settings", "Bookmarks", "History"
            ],
            "office_app": [
        ...
        base_strings = category_strings.get(template.category, [
            "Application", "Program", "Software", "Version", "Copyright"
        ])
        
        # Add some random benign strings
        additional_strings = [
            "Version 1.0", "Copyright 2024", "All rights reserved",
            "Microsoft Corporation", "Windows", "System32", "Program Files"
        ]
    ...
    def save_templates(self) -> None:
        ...
        try:
            ...
            logger.info("action=templates_saved path=%s count=%d", 
                       self.template_db_path, len(self.templates))
    ...
    def add_template_from_pe(self, pe_data: bytes, name: str, category: str) -> bool:
        ...
        try:
            ...
            template = BenignTemplate(
                ...
                common_functions=[imp.function_name for imp in imports[:20]]  # Limit to 20
            )
