...
class PEWriter:
    ...
    def __init__(self, pe_data: bytes):
        ...
        logger.info("action=pe_writer_initialized size=%d machine=0x%x", 
                   len(pe_data), self.pe.FILE_HEADER.Machine)
    ...
    def add_section(self, name: str, data: bytes, characteristics: int = 0x40000000) -> bool:
        ...
        virtual_size = ((len(data) + section_alignment - 1) // section_alignment) * section_alignment
        ...
        if last_section:
            ...
            virtual_address = last_section.VirtualAddress + last_section.Misc_VirtualSize
        else:
        ...
        new_section = SectionStructure(
            self.pe.__IMAGE_SECTION_HEADER_format__,
            pe=self.pe
        )
        ...
        logger.info("action=section_added name=%s size=%d virtual_addr=0x%x", 
                   name, len(data), virtual_address)
    ...
    def modify_section_data(self, section_name: str, new_data: bytes) -> bool:
        ...
        if len(new_data) > section.SizeOfRawData:
            logger.warning("action=data_too_large section=%s available=%d needed=%d", 
                          section_name, section.SizeOfRawData, len(new_data))
            return False
        ...
        logger.info("action=section_modified name=%s size=%d", 
                   section_name, len(new_data))
        
        return True
    
    def inject_payload_to_section(self, section_name: str, payload: bytes, 
                                 offset: int = 0) -> bool:
        ...
        if offset + len(payload) > section.SizeOfRawData:
            logger.warning("action=payload_too_large section=%s offset=%d available=%d needed=%d", 
                          section_name, offset, section.SizeOfRawData - offset, len(payload))
            return False
        ...
        logger.info("action=payload_injected section=%s offset=%d size=%d", 
                   section_name, offset, len(payload))
    ...
    def add_import(self, dll_name: str, function_name: str) -> bool:
        ...
        logger.warning("action=import_addition_not_implemented dll=%s function=%s", 
    ...
    def modify_strings(self, string_replacements: Dict[str, str]) -> int:
        ...
        for old_string, new_string in string_replacements.items():
            ...
            if len(new_bytes) > len(old_bytes):
                logger.warning("action=string_too_long old=%s new=%s", 
    ...
    def add_junk_data(self, section_name: str, size: int) -> bool:
        ...
        null_start = self.pe_data.find(b'\x00' * size, section_data_start, section_data_end)
        if null_start != -1:
            self.pe_data[null_start:null_start + size] = junk_data
            logger.info("action=junk_data_added section=%s size=%d", 
                       section_name, size)
            return True
        else:
            logger.warning("action=no_space_for_junk section=%s size=%d", 
                          section_name, size)
            return False
    ...
    def _find_section(self, section_name: str) -> Optional[SectionStructure]:
        ...
        for section in self.pe.sections:
            if section.Name.decode('utf-8', errors='ignore').rstrip('\x00') == section_name:
                return section
        return None
