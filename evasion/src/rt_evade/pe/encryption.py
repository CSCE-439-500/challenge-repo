    ...
    def encrypt_pe(self, pe_data: bytes) -> bytes:
        ...
        try:
            ...
            if not code_sections:
                logger.info(
                    "action=code_encryption_skipped reason=no_code_sections_found"
                )
                return pe_data
            ...
            with PEWriter(pe_data) as writer:
                for section in code_sections:
                    ...
                    # Get section data
                    section_data = pe_data[
                        section.raw_address : section.raw_address + section.raw_size
                    ]
                ...
                metadata = self._create_encryption_metadata(
                    encryption_key, code_sections
                )
                writer.add_section(
                    ".encrypted", metadata, characteristics=0x40000000
                )  # IMAGE_SCN_CNT_INITIALIZED_DATA

                result = writer.get_modified_data()

            logger.info(
                "action=code_encryption_applied sections=%d algorithm=%s",
                len(code_sections),
