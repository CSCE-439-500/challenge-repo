...
class PESectionManipulator:
    ...
    def manipulate_sections(self, pe_data: bytes) -> bytes:
        ...
        try:
            ...
            logger.info(
                "action=section_manipulation_applied padding=%s entropy=%s",
                self.config.enable_section_padding,
                self.config.enable_entropy_increase,
            )
    ...
    def _apply_section_padding(self, writer: PEWriter) -> None:
        ...
        for section_name in self.config.padding_sections:
            try:
                ...
                logger.debug(
                    "action=section_padding_applied section=%s size=%d",
                    section_name,
                    junk_size,
                )
            except Exception as e:
    ...
    def _apply_entropy_increase(self, writer: PEWriter) -> None:
        ...
        for section_name in self.config.entropy_sections:
            try:
                ...
                logger.debug(
                    "action=entropy_increase_applied section=%s size=%d",
                    section_name,
                    len(entropy_data),
                )
            except Exception as e:
    ...
    def get_section_manipulation_report(
        self, original_data: bytes, manipulated_data: bytes
    ) -> Dict[str, Any]:
