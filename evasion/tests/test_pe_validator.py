"""Tests for PE validator functionality.

This module contains pytest tests for the PE validator component,
including PE format validation and execution compatibility checking.
"""
...
from rt_evade.pe.validator import PEValidator, PEValidationError
...
class TestPEValidator:
    """Test PE validator functionality."""
    
    def test_pe_validator_initialization(self):
        """Test PE validator initialization."""
        with patch.dict(os.environ, {"REDTEAM_MODE": "true"}):
            validator = PEValidator()
            assert validator is not None
    
    def test_pe_validator_requires_redteam_mode(self):
        """Test that PE validator requires REDTEAM_MODE."""
        with patch.dict(os.environ, {}, clear=True):
            with pytest.raises(RuntimeError, match="REDTEAM_MODE not enabled"):
                PEValidator()
    
    def test_validate_pe(self, mock_pe_data):
        """Test PE validation."""
        with patch.dict(os.environ, {"REDTEAM_MODE": "true"}):
            validator = PEValidator()
            result = validator.validate_pe(mock_pe_data)
            assert isinstance(result, dict)
            assert "valid" in result
            assert "errors" in result
            assert "warnings" in result
    
    def test_validate_execution_compatibility(self, mock_pe_data):
        """Test execution compatibility validation."""
        with patch.dict(os.environ, {"REDTEAM_MODE": "true"}):
            validator = PEValidator()
            result = validator.validate_execution_compatibility(mock_pe_data)
            assert isinstance(result, bool)
    
    def test_get_validation_summary(self, mock_pe_data):
        """Test validation summary generation."""
        with patch.dict(os.environ, {"REDTEAM_MODE": "true"}):
            validator = PEValidator()
            validator.validate_pe(mock_pe_data)
            summary = validator.get_validation_summary()
            assert isinstance(summary, str)
            assert "PE Validation" in summary
