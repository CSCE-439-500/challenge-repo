"""Tests for refactored PE obfuscator functionality.

This module contains pytest tests for the refactored PE obfuscator component,
focusing on integration and configuration testing.
"""
...
class TestPEObfuscatorRefactored:
    """Test refactored PE obfuscator functionality."""
    ...
    def test_obfuscator_initialization(self):
        ...
        with patch.dict(os.environ, {"REDTEAM_MODE": "true"}):
            ...
            # Check that specialized obfuscators are initialized
            assert obfuscator.compressor is not None
            assert obfuscator.encryptor is not None
            assert obfuscator.string_obfuscator is not None
            assert obfuscator.section_manipulator is not None
    ...
    def test_obfuscate_pe_with_specialized_modules(self, mock_pe_data):
        """Test PE obfuscation using specialized modules."""
        with patch.dict(os.environ, {"REDTEAM_MODE": "true", "ALLOW_ACTIONS": "true"}):
            config = PEObfuscationConfig(
                ...
                enable_detection_mitigation=False,
                # Sub-configurations
                ...
                string_obfuscation_config=StringObfuscationConfig(
                    enable_string_obfuscation=True,
                    obfuscation_method="base64",
                    min_string_length=4
                ),
                section_manipulation_config=SectionManipulationConfig(
                    enable_section_padding=True,
                    enable_entropy_increase=True,
            ...
            try:
                ...
                assert "Invalid PE file" in str(e) or "PE format error" in str(e) or "Writes disabled" in str(e)
    ...
    def test_get_obfuscation_report_with_specialized_modules(self, mock_pe_data):
        """Test generating obfuscation report with specialized modules."""
        with patch.dict(os.environ, {"REDTEAM_MODE": "true"}):
            config = PEObfuscationConfig(
                enable_compression=True,
                enable_code_encryption=True,
                ...
                enable_section_padding=True,
                enable_entropy_increase=True,
                compression_config=CompressionConfig(enable_compression=True),
                encryption_config=EncryptionConfig(enable_code_encryption=True),
                string_obfuscation_config=StringObfuscationConfig(enable_string_obfuscation=True),
                section_manipulation_config=SectionManipulationConfig(
                    enable_section_padding=True,
                    enable_entropy_increase=True
                )
            ...
            # Check that specialized module reports are included
            if config.enable_compression:
                assert "compression" in report
            if config.enable_code_encryption:
                assert "encryption" in report
            if config.enable_string_obfuscation:
                assert "string_obfuscation" in report
            if config.enable_section_padding or config.enable_entropy_increase:
                assert "section_manipulation" in report


class TestPEObfuscationConfigRefactored:
    """Test refactored PE obfuscation configuration."""
    ...
    def test_default_config(self):
        ...
        assert config.compression_config is None
        assert config.encryption_config is None
        assert config.string_obfuscation_config is None
        assert config.section_manipulation_config is None
    
    def test_custom_config_with_sub_configs(self):
        """Test custom configuration with sub-configurations."""
        compression_config = CompressionConfig(
            enable_compression=True,
        ...
        string_config = StringObfuscationConfig(
            enable_string_obfuscation=True,
            obfuscation_method="xor",
            min_string_length=8
        )
        section_config = SectionManipulationConfig(
            enable_section_padding=True,
        ...
        config = PEObfuscationConfig(
            ...
            string_obfuscation_config=string_config,
            section_manipulation_config=section_config
        )
        ...
        assert config.enable_compression is True
        assert config.enable_code_encryption is True
        assert config.enable_import_manipulation is False
        assert config.enable_static_evasion is False
        assert config.enable_detection_mitigation is False
        assert config.target_category == "system_utility"
        assert config.max_file_size == 1024 * 1024
        
        # Check sub-configurations
        assert config.compression_config == compression_config
        assert config.encryption_config == encryption_config
        assert config.string_obfuscation_config == string_config
        assert config.section_manipulation_config == section_config
