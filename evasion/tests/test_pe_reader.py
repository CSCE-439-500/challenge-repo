"""Tests for PE reader functionality.

This module contains pytest tests for the PE reader component,
including PE file parsing, header extraction, and section analysis.
"""
...
from rt_evade.pe.reader import PEReader, PEHeaderInfo, PESectionInfo, PEImportInfo
...
class TestPEReader:
    """Test PE reader functionality."""
    
    def test_pe_reader_initialization(self, mock_pe_data):
        """Test PE reader initialization."""
        with patch.dict(os.environ, {"REDTEAM_MODE": "true"}):
            reader = PEReader(mock_pe_data)
            assert reader is not None
            reader.close()
    
    def test_pe_reader_requires_redteam_mode(self, mock_pe_data):
        """Test that PE reader requires REDTEAM_MODE."""
        with patch.dict(os.environ, {}, clear=True):
            with pytest.raises(RuntimeError, match="REDTEAM_MODE not enabled"):
                PEReader(mock_pe_data)
    
    def test_get_header_info(self, mock_pe_data):
        ...
        with patch.dict(os.environ, {"REDTEAM_MODE": "true"}):
            with PEReader(mock_pe_data) as reader:
                ...
                assert header_info.machine == 0x014c  # x86
                assert header_info.subsystem == 2  # GUI
    
    def test_get_sections(self, mock_pe_data):
        """Test getting section information."""
        with patch.dict(os.environ, {"REDTEAM_MODE": "true"}):
            with PEReader(mock_pe_data) as reader:
                sections = reader.get_sections()
                assert isinstance(sections, list)
                if sections:  # If pefile can parse our mock data
                    assert isinstance(sections[0], PESectionInfo)
    
    def test_get_imports(self, mock_pe_data):
        """Test getting import information."""
        with patch.dict(os.environ, {"REDTEAM_MODE": "true"}):
            with PEReader(mock_pe_data) as reader:
                imports = reader.get_imports()
                assert isinstance(imports, list)
                # Mock data doesn't have imports, so list should be empty
                assert len(imports) == 0
    
    def test_get_strings(self, mock_pe_data):
        """Test getting strings from PE."""
        with patch.dict(os.environ, {"REDTEAM_MODE": "true"}):
            with PEReader(mock_pe_data) as reader:
                strings = reader.get_strings()
                assert isinstance(strings, list)
    
    def test_get_entropy_analysis(self, mock_pe_data):
