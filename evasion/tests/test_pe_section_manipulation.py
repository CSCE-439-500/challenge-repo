...
from rt_evade.pe.section_manipulation import PESectionManipulator, SectionManipulationConfig
...
class TestPESectionManipulator:
    ...
    def test_manipulator_initialization(self):
        ...
        with patch.dict(os.environ, {"REDTEAM_MODE": "true"}):
            ...
            assert manipulator.config.enable_section_padding == config.enable_section_padding
            assert manipulator.config.enable_entropy_increase == config.enable_entropy_increase
    ...
    def test_get_section_manipulation_report(self, mock_pe_data):
        ...
        with patch.dict(os.environ, {"REDTEAM_MODE": "true"}):
            ...
            try:
                result = manipulator.manipulate_sections(mock_pe_data)
                report = manipulator.get_section_manipulation_report(mock_pe_data, result)
