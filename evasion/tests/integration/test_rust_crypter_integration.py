...
class TestRustCrypterIntegration:
    ...
    def test_encrypt_pe_file_success(
        self,
        mock_unlink,
        mock_read_bytes,
        mock_write_bytes,
    ...
    def test_generate_stub_success(
        self,
        mock_stat,
        mock_unlink,
        mock_read_bytes,
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            stub_path = integration.generate_stub(
                encrypted_bytes, key_bytes, output_path
            )
    ...
    def test_get_encryption_report(self):
        ...
        try:
            ...
            report = integration.get_encryption_report(
                original_data, encrypted_data, stub_path
            )
...
class TestRustCrypterIntegrationEdgeCases:
    ...
    @patch("rt_evade.dropper.rust_crypter.Path.write_bytes")
    @patch("rt_evade.dropper.rust_crypter.Path.unlink")
    def test_encryption_timeout(
        self, mock_unlink, mock_write_bytes, mock_copy2, mock_exists, mock_run
    ):
    ...
    def test_encryption_failure(
        ...
        mock_run.side_effect = [
            ...
            subprocess.CalledProcessError(
                1, "cargo", stderr="Encryption failed"
            ),  # encryption failure
        ]
        mock_unlink.return_value = None
